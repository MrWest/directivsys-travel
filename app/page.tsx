      ...
      <section className="relative h-[85vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-20 text-center px-4 max-w-4xl mx-auto">
          ...
          <h1 className="text-5xl md:text-7xl font-bold text-white mb-6 leading-tight">
            Your Barbados
            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-cyan-400">Dream Trip</span>
          </h1>
          <p className="text-xl text-slate-600 mb-8 max-w-2xl mx-auto">
            Book hotels, flights, transfers, and dining — all through a single AI-powered conversation.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/hotels" className="px-8 py-4 rounded-xl bg-sky-500 hover:bg-sky-400 text-white font-bold text-lg transition-all">Explore Hotels</Link>
            <Link href="/login" className="px-8 py-4 rounded-xl border border-white/40 hover:border-white/70 text-white font-medium text-lg transition-all hover:bg-white/5">Sign In to Book</Link>
          </div>
        </div>
      </section>
          ...
          {featuredHotels.map(hotel => (
            <Link key={hotel.id} href={`/hotels/${hotel.id}`} className="group rounded-2xl overflow-hidden bg-white/80 border border-slate-200 hover:border-sky-200 transition-all">
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-3 right-3 px-2 py-1 rounded-lg bg-white/80 text-sky-600 text-xs font-medium">{'★'.repeat(hotel.stars)}</div>
              </div>
              <div className="p-4">
                <h3 className="text-slate-700 font-bold text-lg mb-1">{hotel.name}</h3>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sky-600 font-bold">${hotel.roomTypes[0].pricePerNight}<span className="text-slate-400 font-normal text-xs">/night</span></span>
                </div>
              </div>
            </Link>
          ))}
          ...
          {featuredRestaurants.map(r => (
            <Link key={r.id} href={`/dining/${r.id}`} className="group rounded-2xl overflow-hidden bg-white/80 border border-slate-200 hover:border-rose-200 transition-all">
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-3 right-3 px-2 py-1 rounded-lg bg-white/80 text-rose-500 text-xs font-medium">{r.priceRange}</div>
              </div>
              <div className="p-4">
                <h3 className="text-slate-700 font-bold text-lg mb-1">{r.name}</h3>
